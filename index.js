...
const ignroeNameList = ['.', '..']
const componentExtensions = ['.vue', '.jsx']
const fileNameList = fs.readdirSync(inDir).filter(name => !ignroeNameList.includes(name) && componentExtensions.some(v => new RegExp(`.+${v}$`).test(name)))
